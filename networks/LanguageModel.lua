...
--[[compute the probability of producing a given word

    Input: word

    Ouput: probability of producing word..' ' assuming the network has already produced a space
]]
function LM:getProb(kwargs,word)
  word = word..' '
  local T = #word + 1
  local start_text = ' ' 
  local verbose = utils.get_kwarg(kwargs, 'verbose', 0)
  ...
  for t = first_t, T do
    ...
    next_char = self:encode_string(string.sub(word,t-1,t-1)):view(1,1)
    prob = prob * probs[next_char[1][1]]
...
function LM:getProbNormed(kwargs,word)
  word = word..' '
  local T = #word + 1
  local start_text = ' ' 
  local verbose = utils.get_kwarg(kwargs, 'verbose', 0)
  ...
  local _, next_char= nil, nil
  local prob = 1
  local space = self:encode_string(' '):view(1,1)
  for t = first_t, T do
    local probs = torch.div(scores, temperature):double():exp():squeeze()
    probs:div(torch.sum(probs))
    ...
    if next_char ~= space then
      prob = prob * probs[next_char[1][1]] / (1-probs[space[1][1]])
    else
      prob = prob * probs[next_char[1][1]]
    end
...
function LM:getMeanEntropy(kwargs,word)
  word = word..' '
  local T = #word + 1
  local start_text = ' ' 
  local verbose = utils.get_kwarg(kwargs, 'verbose', 0)
  ...
  local _, next_char, prob = nil, nil, nil
  local entropy = 0
  for t = first_t, T do
    local probs = torch.div(scores, temperature):double():exp():squeeze()
    probs:div(torch.sum(probs))
...
--[[compute the probability of ending a given word

    Input: word

    Ouput: probability of producing a space assuming the network has already produced ' word'
]]
function LM:getProbEnd(kwargs,word)
  local T = #word + 2
  local start_text = ' '..word
  local verbose = utils.get_kwarg(kwargs, 'verbose', 0)
  local sample = utils.get_kwarg(kwargs, 'sample', 1)
  local temperature = utils.get_kwarg(kwargs, 'temperature', 1)
  ...
  local scores
  local x = self:encode_string(start_text):view(1, -1) --creates a tensor of size (1,length(start_text))
  local T0 = x:size(2) --length of start_text
  sampled[{{}, {1, T0}}]:copy(x) --the begining of sample takes the values of x
  scores = self:forward(x)[{{}, {T0, T0}}]
  ...
  next_char = self:encode_string(' '):view(1,1)
  prob = probs[next_char[1][1]]
