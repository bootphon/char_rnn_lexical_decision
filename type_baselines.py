"""
Compute the baselines on types 
Warning: should be merged with baselines
"""
import re
import data_loader
import json
...
types = set(re.split('\s',train))
train = ' '.join(list(types))
N = len(train)
    ...
    def loss(current_c, next_c, model):
        if model == unigrams:
            ...
            if current_bigram not in bigrams:
                ...
                if current_c not in unigrams:
                    temp = 0 
                else:
                    temp = -bigrams[current_bigram]/unigrams[current_c]
            loss = temp + np.log(sum([np.exp(bigrams[b]/unigrams[current_c]) for b in possible_bigrams]))
        return loss
...
unigram_file = args.dump_folder.rstrip('/') + '/' + 'type_unigram.json'
bigram_file = args.dump_folder.rstrip('/') + '/' + 'type_bigram.json'
...
print 'Creation of type_unigram_table.txt...'
unigram_table_file = args.dump_folder.rstrip('/') + '/' + 'type_unigram_table.txt'
...
print 'Creation of type_bigram_table.txt...'
bigram_table_file = args.dump_folder.rstrip('/') + '/' + 'type_bigram_table.txt'
